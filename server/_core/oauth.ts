...
export function registerOAuthRoutes(app: any) {
  app.get("/api/oauth/mock-login", async (req: any, res: any) => {
    // Block mock-login in production
    if (process.env.NODE_ENV === "production") {
      res.status(404).json({ error: "Not found" });
      return;
    }
    // In mock-login, we can just bypass the CSRF state check to make local dev easier.
    try {
